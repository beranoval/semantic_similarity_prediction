...
class NormOfVector():
    ...
    def get_norm_of_embeddings(self):
        # Výpočet normy jednotlivých vektorů
    ...
    def check_if_embeddings_are_normalized(self):
        # Kontrola, zda jsou všechny normy blízké 1
...
class FastTextEmbeddings:
    ...
    def train_model(self, window, min_count, seed, sg, vector_size, norm, sample, epochs):
    ...
    def train_and_get_emb(self, words, window, min_count, seed, sg, vector_size, norm, sample, epochs):
        self.train_model(window, min_count, seed, sg, vector_size, norm, sample, epochs)
...
class Similarity2:
    ...
    def get_cosine_similarity_of_all_words(self):
        ...
        # Create a mask to exclude the diagonal and duplicate values
        mask = np.triu(np.ones(cos_sim_matrix.shape), k=1).astype(bool)
        # Extract non-diagonal, non-duplicate values and their corresponding indices
